...
The workflow `.github/workflows/news-research-cycle.yml` runs every two hours at minute 17. GitHub schedules execute only after the workflow is present on the default branch, and GitHub may delay cron jobs.
...
Before enabling cron, run the workflow manually once. Confirm that the News Signal Lab shows PostgreSQL, a completed cycle, and expected ticker coverage. A manual dispatch still requires the database secret even when the cron enable variable is false.
